...
const SignUp = () => {
  const { connect, wallet, select, publicKey } = useWallet();
  const { requestGatewayToken, gatewayToken, gatewayStatus } = useGateway();
  const connection = new web3.Connection(web3.clusterApiUrl('devnet'));
  useEffect(() => {
    const req = !publicKey && wallet && wallet.readyState === 'Installed';
    if (req) {
      try {
        connect();
      } catch (e) {
  ...
  const CivicLoginClickHandler = () => {
    console.log(
      'user pub key before civic pass requests gateway token - ',
      publicKey?.toBase58()
    );
    requestGatewayToken()
      .then(() =>
        console.log('civic gateway token - ', gatewayToken, gatewayStatus)
      )
      .catch((e) => console.log('civic error - ', e));
  };
  return (
    <Container py={{ base: '2rem', md: '3rem' }} maxW="full" height="200rem">
      ...
      >
        ...
        <Text color="#C5C5C5" fontSize={{ base: 'xs', md: 'sm' }}>
          Sign in with your favorite service provider and create your cubic
          account.
        </Text>
        <VStack w={{ base: '15rem', md: '16rem' }} gap="0.4rem" py="1.4rem">
          <SolanaWalletButton />
